//
//  Sub_SMFMoviePreviewController.m
//  SMFramework
//
...
@implementation Sub_SMFMoviePreviewController
@synthesize delegate;
...
-(NSMutableDictionary *)getInformation
{
    NSMutableDictionary *d = [[NSMutableDictionary alloc]initWithObjectsAndKeys:
                              ...
                              @"pg",kSMFMovieRating,
                              nil];
    if (self.datasource!=nil && [self.datasource conformsToProtocol:@protocol(Sub_SMFMoviePreviewControllerDatasource)]) {
...
-(void)reload
{
     ...
     *  The Title
     */
    checkNil2(_metadataTitleControl);
    ...
    for(int counter=0;counter<4;counter++)
    {
        ...
        [head setText:[headers objectAtIndex:counter] withAttributes:[Sub_SMFMoviePreviewController columnHeaderAttributes] ];
        CGRect headFrame;
        ...
        for (int objcount=0; objcount<maxObj; objcount++) {
            if ([[current objectAtIndex:objcount] isKindOfClass:[NSArray class]]) {
                ...
                for (int i=0;i<[objects count];i++)
                {
                    ...
                    {
                        ctrl = [[BRTextControl alloc]init];
                        [ctrl setText:[objects objectAtIndex:i] withAttributes:[Sub_SMFMoviePreviewController columnLabelAttributes]];
                        r.size=[ctrl renderedSize];
                        r.origin.y=tempY-r.size.height;
                        if (r.size.width+r.origin.x>maxX) {
                            r.size.width=maxX-r.origin.x;
                ...
                if ([[current objectAtIndex:objcount] isKindOfClass:[NSString class]]) {
                    ...
                    [(BRTextControl *)obj setText:[current objectAtIndex:objcount] withAttributes:[Sub_SMFMoviePreviewController columnLabelAttributes]];
     ...
     */
	checkNil2(_previousArrowImageControl);
	checkNil2(_nextArrowImageControl);
...
-(BOOL)brEventAction:(BREvent *)action
{
    ...
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(Sub_SMFMoviePreviewControllerDelegate)]) {
    }
    if (remoteAction==kBREventRemoteActionPlay && 
        self.delegate!=nil && 
        action.value==1 && 
        [self.delegate conformsToProtocol:@protocol(Sub_SMFMoviePreviewControllerDelegate)]&&
